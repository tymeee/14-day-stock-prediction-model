...
if uploaded_file is not None:
    ...
    if not required_cols.issubset(df.columns):
        ...
        try:
            ...
            N = 10  # sequence length used in training
            if len(df) < N:
                st.error(f"Not enough rows with valid indicators. Need at least {N}, got {len(df)}.")
            else:
                # Select only indicator columns
                cols_to_use = list(df.columns.difference(['Date']))  # includes raw + indicators
                latest_seq = df[cols_to_use].iloc[-N:].values.flatten()
                ...
                xgb_pred = xgb_pred * 100  # Convert to percentage
